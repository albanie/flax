...
class TestModule:
  ...
  def test_update_static_state_submodules(self):
    ...
    m1 = Foo()
    graphdef, state = nnx.split(m1)
    m2, mapping = nnx.merge(graphdef, state, mapping=True)
    m2.a.add_field()
    new_graphdef, state = nnx.split(m2, mapping=mapping)

    nnx.full_update(graphdef, new_graphdef, state)

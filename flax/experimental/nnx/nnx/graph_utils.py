...
class GraphDef(tp.Generic[Node], reprlib.Representable):
  ...
  def __nnx_repr__(self):
    ...
    yield reprlib.Attr('ref_to_index', self.flatten_ref_to_index)

  def __deepcopy__(self, memo=None):
    nodedef = deepcopy(self.nodedef, memo)
    index_mapping = deepcopy(self.index_mapping, memo)
    if self.flatten_ref_to_index is None:
      ref_to_index = None
    else:
      ref_to_index = RefMap(self.flatten_ref_to_index)
    return GraphDef(nodedef, ref_to_index, index_mapping)

  def __hash__(self):
    # ref_to_index is opaque
    return hash((self.nodedef, self.index_mapping))

  def __eq__(self, other):
    # ref_to_index is opaque
    return (
      isinstance(other, GraphDef)
      and self.nodedef == other.nodedef
      and self.index_mapping == other.index_mapping
    )
...
def _graphdef_flatten(graphdef: GraphDef[Node]):
  # ref_to_index is opaque, we don't propagate it
...
def _graphdef_unflatten(
  ...
  return GraphDef(nodedef, RefMap(), index_mapping)
...
def graph_flatten(
  ...
  /,
  *,
  index_to_ref: dict[Index, tp.Any] | None = None,
) -> tuple[GraphDef[Node], State, tp.Mapping[tp.Any, Index]]:
  ref_to_index = RefMap[tp.Any, Index]()
  flat_state: dict[PathParts, StateLeaf] = {}
  nodedef = _graph_flatten((), ref_to_index, flat_state, x)
  assert not isinstance(nodedef, int)
  if index_to_ref is not None:
    index_to_index = compose_mapping(index_to_ref, ref_to_index)
  else:
    index_to_index = None
  graphdef = GraphDef(nodedef, ref_to_index, index_to_index)
...
def _graph_flatten(
  ...
  for key, value in values:
    if is_node(value):
      nodedef = _graph_flatten((*path, key), ref_to_index, flat_state, value)
...
def graph_unflatten(
  ...
  *,
  ...
  node = _graph_unflatten(
    graphdef.nodedef, state.raw_mapping, index_to_ref, ref_cache
  )
...
def _graph_unflatten(
  ...
  if isinstance(node_impl, GraphNodeImpl):
    ...
    if ref_cache is not None and nodedef.index in ref_cache:
      node = ref_cache[nodedef.index]
...
@tp.overload
def split(
  graph_node: A, mapping: dict[Index, tp.Any] | None = None
) -> tuple[GraphDef[A], State]:
  ...
...
def split(
  ...
  /,
  mapping: dict[Index, tp.Any] | None = None,
) -> tuple[GraphDef[A], State]:
  ...
...
def split(
  ...
  *filters: filterlib.Filter,
  ...
  graphdef, state, _ = graph_flatten(graph_node, index_to_ref=mapping)
...
def merge(
  ...
  *states: State,
  mapping: bool = False,
  ref_cache: dict[Index, tp.Any] | None = None,
) -> A | tuple[A, dict[Index, tp.Any]]:
  # TODO: add docstring of example usage
  if states:
    state = State.merge(state, *states)

  node, index_to_ref = graph_unflatten(graphdef, state)
  index_to_ref: dict[Index, tp.Any] = {}
  node = _graph_unflatten(
    graphdef.nodedef, state.raw_mapping, index_to_ref, ref_cache
  )
...
def full_update(
  current_graphdef, new_graphdef: GraphDef[A], state: State, /, *states: State
):
  if current_graphdef.flatten_ref_to_index is None:
    raise ValueError('Cannot update a graphdef without ref_to_index.')
  ...
  index_to_ref = compose_mapping_reversed(
    current_graphdef.flatten_ref_to_index, new_graphdef.index_mapping
  )
  return graph_unflatten(new_graphdef, state, ref_cache=index_to_ref)[0]
